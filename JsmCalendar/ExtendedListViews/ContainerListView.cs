...
namespace JsmMind
{
	...
	[DesignTimeVisible(false), TypeConverter("JsmMind.ListViewItemConverter")]
	...
	[DesignTimeVisible(false), TypeConverter("JsmMind.SubListViewItemConverter")]
	public class ContainerSubListViewItem: ICloneable
	{
        ...
        //Add By T.L
        public List<int> ColorIndex = null;
        public List<int> ColorValue = null;
	...
	[DesignTimeVisible(false), TypeConverter("JsmMind.ToggleColumnHeaderConverter")]
	...
	public class ColumnHeaderCollection: CollectionBase
	{
		...
		public virtual int Add(JsmMind.ToggleColumnHeader colhead)
		{
		...
		public virtual void AddRange(JsmMind.ToggleColumnHeader[] items)
		{
			lock(List.SyncRoot)
			{
				for (int i=0; i< items.Length; i++)
	...
	public class ContainerListView: Control
	{
		...
		protected JsmMind.ContainerListViewItem focusedItem;	
        ...
        protected Brush rowSelectBrush = SystemBrushes.HighlightText;

        protected Color taskEventSelectColor = Color.LightSteelBlue;
        protected Brush taskEventSelectBrush = SystemBrushes.HighlightText;
        ...
        protected bool separateLine = false;
        

        protected bool editStyle = false;
        protected bool bCompareSort = false;
		...
		private void Construct()
		{
			...
			columns = new JsmMind.ColumnHeaderCollection();
		...
		public JsmMind.ColumnHeaderCollection Columns
		{
			get { return columns; }
		}
        ...
        [
        Category("Appearance"),
        Description("Specifies the brush used for selected rows."),
        DefaultValue(typeof(Brush), "SystemBrushes.HighlightText")
        ]
        public Brush RowSelectBrush
        {
            get { return rowSelectBrush; }
            set { rowSelectBrush = value; }
        }
        ...
        [
        Category("Behavior"),
        Description("Specifies the node color used for separate."),
        DefaultValue(false)
        ]
        public bool SeparateLine
        {
            get { return separateLine; }
            set { separateLine = value; }
        }
        ...
        [
        Category("Behavior"),
        Description("Specifies wether to show edit style"),
        DefaultValue(false)
        ]
        ...
        [
        Category("Behavior"),
        Description("Specifies wether to sort enable"),
        DefaultValue(false)
        ]
        public bool SortEnable
        {
            get { return sortEnable; }
            set { sortEnable = value; }
        }


        public bool BCompareSort
        {
            get { return bCompareSort; }
            set { bCompareSort = value; }
        }
		...
		public bool	VisualStyles
		{
			get 
			{
				...
				try
				{
					val = visualStyles && JsmMind.uxTheme.Wrapper.IsAppThemed();
				}
				catch
				{
					val = visualStyles;
            ...
            if (headerStyle != ColumnHeaderStyle.None)
            {
                ...
                for (i = 0; i < columns.Count; i++)
                {
                        ...
                        && (lp_scr + last < r.Left + r.Width - 2))
                    {
                        ...
                        //draw sort arrow  ,tl 2014/10/9 
                        if (sortEnable && sortColumn == i)
                        {
                            float arrow_x = (float)(lp_scr + last + columns[i].Width - 8 - 4);
                            float arrow_y = (float)(r.Top + 8);
                            Pen arrowPen = new Pen(SystemBrushes.ControlDark, 1.0f);

                            if (sortAsc)
                            {
                                //sort asc
                                g.DrawLine(arrowPen, arrow_x - 5, arrow_y + 5, arrow_x, arrow_y);
                                g.DrawLine(arrowPen, arrow_x - 4, arrow_y + 5, arrow_x, arrow_y + 1);
                                g.DrawLine(arrowPen, arrow_x, arrow_y, arrow_x + 5, arrow_y + 5);
                                g.DrawLine(arrowPen, arrow_x - 5, arrow_y + 5, arrow_x - 3, arrow_y + 5);
                            }
                            else
                            {
                                //sort desc
                                g.DrawLine(arrowPen, arrow_x, arrow_y + 5, arrow_x - 5, arrow_y);
                                g.DrawLine(arrowPen, arrow_x, arrow_y + 4, arrow_x - 4, arrow_y);
                                g.DrawLine(arrowPen, arrow_x - 5, arrow_y, arrow_x + 5, arrow_y);
                                g.DrawLine(arrowPen, arrow_x + 5, arrow_y, arrow_x + 4, arrow_y + 1);
                            }
                        }
                    }
                                ...
                                && (tp_scr + (rowHeight * i) + 2 < r.Top + r.Height - 2 - hsize))
                            {
                                ...
                                else
                                {
                                    ...
                                    else if (columns[j + 1].TextAlign == HorizontalAlignment.Right)
                                    {
                                        ...
                                        g.DrawString(sp, this.Font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(ForeColor)), (float)(lp_scr + last + columns[j + 1].Width - Helpers.StringTools.MeasureDisplayStringWidth(g, sp, this.Font) - 2), (float)(tp_scr + (rowHeight * i) + 2));
                                    }
                                    else
                                    {
                                        sp = TruncatedString(items[i].SubItems[j].Text, columns[j + 1].Width, 12, g);
                                        g.DrawString(sp, this.Font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(ForeColor)), (float)(lp_scr + last + (columns[j + 1].Width / 2) - (Helpers.StringTools.MeasureDisplayStringWidth(g, sp, this.Font) / 2)), (float)(tp_scr + (rowHeight * i) + 2));
                                    }
                                }
                            }
                        }
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
				...
				try
				{
					...
					for (i=0; i<columns.Count; i++)
					{
						colwid = (columns[i].ScaleStyle == ColumnScaleStyle.Spring ? springWid : columns[i].Width);
						if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Pressed)
							JsmMind.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "PRESSED", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,
								lp, tp, r.Width-6, headerBuffer);
						else if (headerStyle != ColumnHeaderStyle.None && columns[i].Hovered)
							JsmMind.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "HOT", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,  
								lp, tp, r.Width-6, headerBuffer);
						else
							JsmMind.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,  
								lp, tp, r.Width-6, headerBuffer);
						last += colwid;
					...
					{
						JsmMind.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc, lp_scr+last, tp, r.Width-last-2+hscrollBar.Value, headerBuffer,  r.Left, r.Top, r.Width, headerBuffer);
					}
				}
				catch
				{
        ...
        protected virtual void DrawHeadersEditStyle(Graphics g, Rectangle r)
        {
            g.FillRectangle(new SolidBrush(SystemColors.Control), r.Left + 2, r.Top + 2, r.Width - 2, headerBuffer);
            ...
            //System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr + last, r.Top + 2, colWidth, r.Top + headerBuffer, ButtonState.Flat);
            g.FillRectangle(SystemBrushes.Control, lp_scr + last, r.Top + 2, colWidth, r.Top + headerBuffer);
            g.DrawLine(Pens.DarkGray, lp_scr + columns[0].Width, r.Top + 1 + headerBuffer, colWidth - columns[0].Width - lp_scr, r.Top + 1 + headerBuffer);

            int textWidth = TextRenderer.MeasureText(g,columns[0].Text, this.Font).Width;

            g.DrawString(columns[0].Text, this.Font, SystemBrushes.ControlText, (float)(lp_scr + last+columns[0].Width), (float)(r.Top + 5));

            // only render trailing column header if the end of the
            // last column ends before the boundary of the listview 
            if (!(lp_scr + colWidth + 5 > r.Left + r.Width))
            {
                g.Clip = new Region(new Rectangle(r.Left + 2, r.Top + 2, r.Width - 5, r.Top + headerBuffer));

                //System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr + last, r.Top + 2, r.Width - (r.Left + last) - 3 + hscrollBar.Value, r.Top + headerBuffer, ButtonState.Flat);
                g.FillRectangle(SystemBrushes.Control, lp_scr + last, r.Top + 2, r.Width - (r.Left + last) - 3 + hscrollBar.Value, r.Top + headerBuffer);
            }
